export default class CardList {
    ...
    parseCard(cardBody, cardSmallText, wrapperId, extraData) {
    ...
    saveToStorage() {
        const data = {};
        this.wrappers.forEach(wrapper => {
            const cards = [...wrapper.querySelectorAll('.card')].map(card => {
                const body = card.querySelector('.card_body').textContent;
                const smallText = card.querySelector('.input_years').textContent;
                const extra = card.extraData;
                return { body, smallText, extra };
            });
            data[wrapper.id] = cards;
        });
