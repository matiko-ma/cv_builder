document.addEventListener("DOMContentLoaded",()=>{document.getElementById("btn_create").addEventListener("click",async e=>{var a,i,d;e.preventDefault();let n={};for(let t=0;t<sessionStorage.length;t++){let e=sessionStorage.key(t);n[e]=sessionStorage.getItem(e)}let o=JSON.parse(n.cardsData||"{}"),r=await fetch("https://timeapi.io/api/Time/current/zone?timeZone=Europe/Moscow"),s=await r.json(),p=(a=n,i=o,d=`${s.year}-${String(s.month).padStart(2,"0")}-${String(s.day).padStart(2,"0")} ${String(s.hour).padStart(2,"0")}:${String(s.minute).padStart(2,"0")}:${String(s.seconds).padStart(2,"0")}`,`
        <html>
        <head>
            <meta charset="UTF-8" />
            <style>
